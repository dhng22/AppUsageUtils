...
/**
 * Helper class for the `AppUsageStat` to calculate usage time and initialize data
...
data class AppUsageStatHelper(
     ...
     */
    internal fun onAppEnd(stamp: Long) {
    ...
    internal fun firstEventExec(eventType: Int, stamp: Long) {
        if (eventType == UsageEvents.Event.ACTIVITY_RESUMED) {
            ...
            helperCalendar.set(Calendar.SECOND, 0)
...
fun AppUsageStatHelper.toAppUsageStat():AppUsageStat{
    return AppUsageStat(packageName,timeStamp,icon!!,name,totalUsedTime)
}
